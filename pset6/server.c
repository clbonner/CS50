...
char* indexes(const char* path)
{
    // concatenate path with indexes
    char* index = malloc(strlen(path) + strlen(INDEXHTML) + 1);
    
    // concatenate index.html
    strcpy(index, path); 
    strcat(index,INDEXHTML);
    
    // check for index.html
    if (access(index, F_OK) == 0)
        return index;
    
    // concatenate index.php
    strcpy(index, path); 
    strcat(index,INDEXPHP);
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    // allocate an initial byte of memory to content
    *content = malloc(sizeof(BYTE));
    
    // loop until all data is read from file/stream
    do
    {
        // read in to buffer
        read = fread(buffer, sizeof(BYTE), BYTES, file);
        
        // keep track of file size
        size += read;
        
        // adjust size of memory and error check
        *content = realloc(*content, size);
        ...
        // set pointer position to the end of the string
        *content += size - read;
        
        // copy buffer and error check
        *content = memcpy(*content, buffer, read);
        if (*content == NULL)
        {
            error(500);
            return false;
...
const char* lookup(const char* path)
{
    // determine mime type by checking file extension at end of path
    if (strcasecmp(strchr(path, 0) - 4, ".css") == 0)
        return strcpy(mime, "text/css");
    if (strcasecmp(strchr(path, 0) - 5, ".html") == 0)
        return strcpy(mime, "text/html");
    if (strcasecmp(strchr(path, 0) - 4, ".gif") == 0)
        return strcpy(mime, "image/gif");
    if (strcasecmp(strchr(path, 0) - 4, ".ico") == 0)
        return strcpy(mime, "image/x-icon");
    if (strcasecmp(strchr(path, 0) - 4, ".jpg") == 0)
        return strcpy(mime, "image/jpeg");
    if (strcasecmp(strchr(path, 0) - 3, ".js") == 0)
        return strcpy(mime, "text/javascript");
    if (strcasecmp(strchr(path, 0) - 4, ".php") == 0)
        return strcpy(mime, "text/x-php");
    if (strcasecmp(strchr(path, 0) - 4, ".png") == 0)
        return strcpy(mime, "image/png");
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    // return error if request line does not start with /
    if (line[4] != '/')
    {
        error(501);
        return false;
    ...
    // get abs_path
    for (index = 0; line[i] != '?' && line[i] != ' '; index++, i++)
    {
    ...
    // check if query supplied
    if (line[i] == '?')
    {
        for (i++, index = 0; line[i] != ' '; i++, index++)
        {
            // check for quotation marks
            if (line[i] == '"')
            {
        ...
        // terminate and copy query string
        temp[index] = '\0';
        strcpy(query, temp);
    }
    
    // zero query if none found
    else
        query[0] = '\0';
    
    // check for incorrect whitespace
    if (line[++i] == ' ')
    {
        error(400);
        return false;
